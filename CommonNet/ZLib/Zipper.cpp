...
// simple interface
bool CZipper::ZipFile(LPCSTR szFilePath, int nLevel)
{
	// make zip path
	...
	if (zip.OpenZip(szZipPath, false))
		return zip.AddFileToZip(szFilePath, false, nLevel);

	return FALSE;
}
	
bool CZipper::ZipFolder(LPCSTR szFilePath, bool bIgnoreFilePath, int nLevel)
{
	// make zip path
	char szDrive[_MAX_DRIVE], szFolder[MAX_PATH], szName[_MAX_FNAME];
	_splitpath(szFilePath, szDrive, szFolder, szName, NULL);
	...
	if (zip.OpenZip(szZipPath, FALSE))
		return zip.AddFolderToZip(szFilePath, bIgnoreFilePath, nLevel);

	return FALSE;
}
	
// works with prior opened zip
bool CZipper::AddFileToZip(LPCSTR szFilePath, bool bIgnoreFilePath, int nLevel)
{
	if (!m_uzFile)
		return FALSE;
...
bool CZipper::AddFileToZip(LPCSTR szFilePath, LPCSTR szRelFolderPath, int nLevel)
{
	if (!m_uzFile)
		return FALSE;
...
bool CZipper::AddFolderToZip(LPCSTR szFolderPath, bool bIgnoreFilePath, int nLevel)
{
	if (!m_uzFile)
		return FALSE;
	...
	if (hSearch != INVALID_HANDLE_VALUE) 
	{
		do 
		{
			if (finfo.cFileName[0] != '.') 
			{
				...
				{
					AddFolderToZip(szItem, bIgnoreFilePath, nLevel);
				}
				else 
					AddFileToZip(szItem, bIgnoreFilePath, nLevel);
			}
		} 
