...
#if !defined(AFX_DES_H__ECCA259F_2163_46B5_BB28_680E54F152CC__INCLUDED_)
#define AFX_DES_H__ECCA259F_2163_46B5_BB28_680E54F152CC__INCLUDED_
...
#endif // !defined(AFX_DES_H__ECCA259F_2163_46B5_BB28_680E54F152CC__INCLUDED_)

/* How to use it *****************************************************************************************
Encrypt:
	** DES 1 **
	1. deskey(key, EN0);		// key is unsigned char[8]
	2. des(src, dest);			// src and dest are unsigned char[8], the address of them can be the same

	** DES 3 **
	1. des3key(key, EN0);		// key is unsigned char[24]
								// we also can use des2key(key, EN0), the key is unsigned char[16]
	2. Ddes(src, dest);			// src and dest are unsigned char[8], the address of them can be the same

Decrypt:
	** DES 1 **
	1. deskey(key, DE1);		// key is unsigned char[8]
	2. des(src, dest);			// src and dest are unsigned char[8], the address of them can be the same

	** DES 3 **
	1. des3key(key, DE1);		// key is unsigned char[24]
								// we also can use des2key(key, EN0), the key is unsigned char[16]
	2. Ddes(src, dest);			// src and dest are unsigned char[8], the address of them can be the same
